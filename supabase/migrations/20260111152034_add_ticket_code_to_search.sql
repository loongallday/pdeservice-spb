-- Add ticket_code to text search in search_tickets function
-- Now searching by keyword will also match ticket codes (e.g., PDE-1, PDE-123)
...
CREATE OR REPLACE FUNCTION search_tickets(
  ...
  -- Text search (searches: ticket_code, ticket.id, ticket.details, site.name, company.name_th, company.name_en)
...
COMMENT ON FUNCTION search_tickets IS 'Comprehensive ticket search. Text search (p_details) searches: ticket_code, ticket ID, ticket details, site name, company names (Thai/English).';
