-- Add p_ticket_id parameter to search_tickets function
-- This allows filtering by ticket ID (exact match)
...
BEGIN
  ...
  WITH filtered_tickets AS (
    ...
    WHERE
      -- Ticket ID filter (exact match)
      (p_ticket_id IS NULL OR t.id = p_ticket_id)
...
COMMENT ON FUNCTION search_tickets IS 'Comprehensive ticket search with server-side filtering. Supports ticket ID filter and date_type for duration searches.';
