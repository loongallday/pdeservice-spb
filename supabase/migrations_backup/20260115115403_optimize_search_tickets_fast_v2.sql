-- =============================================
-- Fully optimized search_tickets_fast v2
-- - Includes district_name directly (no second query needed)
-- - Uses aggregation instead of correlated subqueries
-- - Single CTE for filtering (used by both count and data)
-- =============================================
...
BEGIN
  ...
  RETURN QUERY
  WITH
  -- Pre-aggregate employee counts (avoids N correlated subqueries)
  emp_counts AS (
  ...
  -- Single filtered set used for both count and data
  filtered AS (
  ...
  LEFT JOIN emp_counts emp ON t.id = emp.ticket_id
  LEFT JOIN cf_counts cf ON t.id = cf.ticket_id;
END;
$$;
...
COMMENT ON FUNCTION search_tickets_fast IS 'Fully optimized ticket search v2 - single query with all data including location names';
