-- Migration: Update fleet notifications to send to superadmins only (level = 3)
-- Previously sent to all appointment approvers
...
COMMENT ON FUNCTION public.notify_fleet_status_change() IS
  'Trigger function to send notifications to superadmins (level 3) when vehicle status changes (departure/arrival at garage)';
