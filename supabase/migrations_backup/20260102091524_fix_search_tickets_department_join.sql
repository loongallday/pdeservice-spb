-- Fix search_tickets function to use correct department join path
-- department is accessed via employee.role_id -> main_org_roles.department_id
...
COMMENT ON FUNCTION search_tickets IS 'Comprehensive ticket search with server-side filtering. Department filter uses role.department_id path.';
