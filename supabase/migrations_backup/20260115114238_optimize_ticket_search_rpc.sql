-- =============================================
-- Optimized ticket search RPC - returns full data in one query
-- =============================================
...
BEGIN
  ...
  SELECT
    ...
    t.site_id,
    s.name as site_name,
    COALESCE(c.name_th, c.name_en) as company_name,
    s.province_code,
    s.district_code,
    ...
    wg.code as work_giver_code,
    wg.name as work_giver_name,
...
COMMENT ON FUNCTION search_tickets_fast IS 'Optimized ticket search - returns full data in single query';
