-- Add ticket ID to text search (p_details parameter)
-- Now searching by keyword will also match ticket UUIDs
...
CREATE OR REPLACE FUNCTION search_tickets(
  ...
  -- Text search (searches: ticket.id, ticket.details, site.name, company.name_th, company.name_en)
...
BEGIN
  ...
  WITH filtered_tickets AS (
    ...
    WHERE
      ...
      -- Text search across multiple fields including ticket ID
      AND (p_details IS NULL OR (
        t.id::text ILIKE '%' || p_details || '%'
...
COMMENT ON FUNCTION search_tickets IS 'Comprehensive ticket search. Text search (p_details) searches: ticket ID, ticket details, site name, company names (Thai/English).';
