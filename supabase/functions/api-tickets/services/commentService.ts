...
export class CommentService {
  ...
  /**
   * Get all comments for a ticket (paginated)
   */
  static async getByTicketId(
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        created_at,
        updated_at,
        author:main_employees!child_ticket_comments_author_id_fkey(
          id,
          name,
          code,
          nickname,
        ...
        photos:child_comment_photos(
          id,
          image_url,
          display_order,
          created_at
        ),
        files:child_comment_files(
          id,
          file_url,
          file_name,
          file_size,
  ...
  static async create(
    ...
    // Create notifications for mentions and previous commenters (async, don't wait)
    NotificationService.createCommentNotifications(
      ...
      console.error('[comment] Failed to create notifications:', err);
    });
    ...
    logTicketAudit({
      ...
      metadata: {
        mentioned_count: mentionedIds.length,
        photo_count: photos.length,
        file_count: files.length,
      },
    }).catch(err => {
      console.error('[comment] Failed to log audit:', err);
    });
