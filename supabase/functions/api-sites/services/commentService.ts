/**
 * Comment Service - Business logic for site comments
 */
...
export class CommentService {
  ...
  /**
   * Get all comments for a site (paginated)
   */
  static async getBySiteId(
    siteId: string,
    ...
    if (siteError || !site) {
      throw new NotFoundError('ไม่พบไซต์ที่ระบุ');
    }

    // Count
    const { count } = await supabase
      .from('child_site_comments')
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        created_at,
        updated_at,
        author:main_employees!child_site_comments_author_id_fkey(
          id,
          name,
          code,
          nickname,
        ...
        photos:child_site_comment_photos(
          id,
          image_url,
          display_order,
          created_at
        ),
        files:child_site_comment_files(
          id,
          file_url,
          file_name,
          file_size,
  ...
  static async create(
    ...
    if (input.photos && input.photos.length > 0) {
      ...
      const { data: insertedPhotos, error: photoError } = await supabase
        .from('child_site_comment_photos')
        .insert(photoRecords)
